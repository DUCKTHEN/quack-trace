...
Quack Trace is a browser-based pattern tracing tool for making garment shapes from images, grid paper, DXF data, or quick sketches.

Quack Trace は、画像・方眼紙・DXF・簡単なスケッチをもとに、服のパーツ形状を作るためのブラウザ製図トレースツールです。
...
- 実寸確認用の方眼紙を使えます。
- Import DXF lines and shapes from CAD tools.
- CADなどで作ったDXFの線や図形を読み込めます。
...
Export behavior is still experimental. Different tools read curves and DXF data differently, so it is best to test with small shapes first.

出力まわりはまだ実験中です。ツールごとにカーブやDXFの読み方が違うので、まずは小さな図形で試すのがおすすめです。
