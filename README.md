...
- Note: MD/CLO Python export approximates curves as sampled point sequences. It does not preserve editable Bezier handles in MD/CLO.
- `MD/CLO py`: Marvelous Designer / CLO 3D に点や線を作るためのPython出力です。
- 注記: MD/CLO Python出力は、カーブを細かい点列として近似します。MD/CLO側で編集できるBezierハンドルを保持する形式ではありません。
